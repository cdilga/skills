...
Five skills for running deep, multi-round, multi-model PR reviews where you stay the reviewer of record — `squint-kickoff` (paste a PR link, get a worktree checkout + ranked findings; works cold, no onboarding required), `squint-deeper` (loop for more findings until dry, with optional steering, rotating lenses including a verification lens), `squint-walkthrough` (step through findings, then post one batched review only after explicit approval — the only skill that posts), `squint-cloud` (run propose-only reviews on GitHub's hosted Copilot agents and harvest them into the same scratch), and `squint-onboard` (one heavy, manual deep pass that produces repo-readiness `AGENTS.md`/`.agents/skills` and/or repo-tailored review skills). Consultant models (Gemini/Codex) only ever propose fixes — nothing edits code, nothing posts without you; per-PR state is disposable and GitHub is the record once posted. See [`skills/reviewing/README.md`](skills/reviewing/README.md) for the working guide and quickstarts.
